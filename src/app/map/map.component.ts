...
export class MapComponent implements OnInit {
  ...
  geocodeEvents(): void {
    ...
    for(let i = 0; i < this.events.length; i++){
      ...
      if(this.events[i].lat === 0 && this.events[i].lng === 0){

        this.geocoder.geocode({'address' : this.events[i].eventAddress }, (results, status) =>{

          this.events[i].lat = results[0].geometry.location.lat();
          this.events[i].lng = results[0].geometry.location.lng();
